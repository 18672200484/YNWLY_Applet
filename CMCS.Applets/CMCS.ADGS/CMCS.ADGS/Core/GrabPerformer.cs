        ...
        private void Grab()
        {
            ...
            foreach (AssayGraber assayGraber in this._ADGSAppConfig.AssayGrabers)
            {
                try
                {
                    ...
                    // 在数据中创建表
                    if (selfDber.Connection.ExecuteScalar<int>(OracleSqlBuilder.BuildHasTableSQL(assayGraber.TableName)) == 0)
                        selfDber.Connection.Execute(OracleSqlBuilder.BuildTableSQL(assayGraber.TableName, dtlAssay));
                    ...
                    foreach (DataRow drAssay in dtlAssay.Rows)
                    {
                        ...
                        if (selfDber.Connection.ExecuteScalar<int>(OracleSqlBuilder.BuildHasRecordSQL(assayGraber.TableName, primaryKeyValue)) == 0)
                        ...
                        syncCount += selfDber.Connection.Execute(execSql);
                    }
