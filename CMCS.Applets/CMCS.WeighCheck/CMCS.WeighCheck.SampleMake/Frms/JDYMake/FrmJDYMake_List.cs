	...
	public partial class FrmJDYMake_List : MetroAppForm
	{
		...
		private void InitHardware()
		{
			try
			{
				...
				{
					// 初始化-读卡器
					success = Hardwarer.ReadRwer.OpenNetPort(CommonDAO.GetInstance().GetAppletConfigString("读卡器IP"), CommonDAO.GetInstance().GetAppletConfigInt32("读卡器端口"));
		...
		private void FrmJDYMake_List_Shown(object sender, EventArgs e)
		{
			InitHardware();
		}
		...
		private void FrmJDYMake_List_FormClosed(object sender, FormClosedEventArgs e)
		{
			Hardwarer.ReadRwer.OnStatusChange -= new HFReaderRwer.StatusChangeHandler(Rwer_OnStatusChange);
		}
