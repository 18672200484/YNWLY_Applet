...
/// 编码打印
/// </summary>
public class CodePrinterMake
{
    Font FontContent = new Font("微软雅黑", 14, FontStyle.Regular, GraphicsUnit.Pixel);
    ...
    List<string> _listType = new List<string>();

    /// <summary>
    /// TicketPrint
    /// </summary>
    /// <param name="pringDoc">PrintDocument</param>
    public CodePrinterMake(PrintDocument pringDoc)
    {
        ...
        this.PringDoc.DefaultPageSettings.PaperSize = new PaperSize("Custum", 320, 140);
    ...
    public void Print(List<string> listCode)
    {
        try
        {
            ...
            foreach (string item in _listType)
            {
                this._listType.Add(item);
            }
            this.PringDoc.Print();
        }
        catch (Exception ex)
    ...
    public void Print(string Code, string Type)
    {
        try
        {
            this._listCode.Clear();
