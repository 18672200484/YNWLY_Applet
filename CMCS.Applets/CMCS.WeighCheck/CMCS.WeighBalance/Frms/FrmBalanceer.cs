...
namespace CMCS.WeighBalance.Frms
{
    public partial class FrmBalanceer : MetroForm
    {
        ...
        {
            List<InfBalanceRecordDetail> list = commonDAO.SelfDber.Entities<InfBalanceRecordDetail>("where BalanceRecordId=:BalanceRecordId", new { BalanceRecordId = balanceId });
            superGridControl.PrimaryGrid.DataSource = list;
        }
        ...
        private bool JoinBalanceDetail(double weight)
        {
            ...
            string GGNo = "1";
            InfBalanceRecordDetail lastdetail = commonDAO.SelfDber.Entity<InfBalanceRecordDetail>("where BalanceRecordId=:BalanceRecordId and SyncFlag=0 order by GGCode desc", new { BalanceRecordId = this.CurrentAssay.Id });
            if (lastdetail != null)
            {
