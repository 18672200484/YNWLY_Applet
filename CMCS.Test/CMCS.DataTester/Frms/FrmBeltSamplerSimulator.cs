...
namespace CMCS.DataTester.Frms
{
    ...
    {
        SqlServerDapperDber dber = new SqlServerDapperDber(CommonDAO.GetInstance().GetCommonAppletConfigString("皮带采样机连接字符串"));
        ...
        private void CmdHandle(EquPDCYJCmd input)
        {
            Task task = new Task((state) =>
            {
                ...
                else if (pDCYJCmd.CmdCode == eEquInfSamplerCmd.结束采样.ToString())
                {
                    ...
                    // 更新系统状态为就绪待机
                    dber.Execute("update " + EntityReflectionUtil.GetTableName<EquPDCYJSignal>() + " set TagValue=@TagValue where MachineCode=@MachineCode and TagName=@TagName", new { MachineCode = pDCYJCmd.MachineCode, TagName = eSignalDataName.程序状态.ToString(), TagValue = eEquInfSamplerSystemStatus.就绪待机.ToString() });
        ...
        private void btnSystemStatus_JXDJ_Click(object sender, EventArgs e)
        {
            ...
            dber.Execute("update " + EntityReflectionUtil.GetTableName<EquPDCYJSignal>() + " set TagValue=@TagValue where MachineCode=@MachineCode and TagName=@TagName", new { MachineCode = (cmbBeltSampler.SelectedIndex + 1).ToString(), TagName = eSignalDataName.设备状态.ToString(), TagValue = eEquInfSamplerSystemStatus.就绪待机.ToString() });
        }
        ...
        private void btnSystemStatus_ZZYX_Click(object sender, EventArgs e)
        {
            ...
            dber.Execute("update " + EntityReflectionUtil.GetTableName<EquPDCYJSignal>() + " set TagValue=@TagValue where MachineCode=@MachineCode and TagName=@TagName", new { MachineCode = (cmbBeltSampler.SelectedIndex + 1).ToString(), TagName = eSignalDataName.设备状态.ToString(), TagValue = eEquInfSamplerSystemStatus.正在运行.ToString() });
        }
        ...
        private void btnSystemStatus_ZZXY_Click(object sender, EventArgs e)
        {
            ...
            dber.Execute("update " + EntityReflectionUtil.GetTableName<EquPDCYJSignal>() + " set TagValue=@TagValue where MachineCode=@MachineCode and TagName=@TagName", new { MachineCode = (cmbBeltSampler.SelectedIndex + 1).ToString(), TagName = eSignalDataName.设备状态.ToString(), TagValue = eEquInfSamplerSystemStatus.正在卸样.ToString() });
        }
