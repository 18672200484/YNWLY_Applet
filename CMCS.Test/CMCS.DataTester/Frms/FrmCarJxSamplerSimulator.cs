    ...
    public partial class FrmCarJxSamplerSimulator : Form
    {
        ...
        private void CmdHandle(EquQCJXCYJUnloadCmd input)
        {
            Task task = new Task((state) =>
            {
                ...
                // 更新系统状态为正在卸样
                this.EquDber.Execute("update " + EntityReflectionUtil.GetTableName<EquQCJXCYJSignal>() + " set TagValue=@TagValue where TagName=@TagName", new { TagName = eSignalDataName.设备状态.ToString(), TagValue = eEquInfSamplerSystemStatus.正在卸样.ToString() });
                Thread.Sleep(3000);
        ...
        private void btnSystemStatus_FSGZ_Click(object sender, EventArgs e)
        {
            ...
            this.EquDber.Execute("update " + EntityReflectionUtil.GetTableName<EquQCJXCYJSignal>() + " set TagValue=@TagValue where  TagName=@TagName", new { TagName = eSignalDataName.设备状态.ToString(), TagValue = eEquInfSamplerSystemStatus.发生故障.ToString() });
        }
