...
namespace CMCS.CarTransport.WeighterHand.Frms.Transport.Print
{
    /// <summary>
    /// 报表打印
    /// </summary>
    class WagonPrinterCollect : MetroAppForm
    {
        ...
        private void _PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            //扣重
            string KsWeightTitle = "扣重(T)";
            SizeF KsWeightSizeTitle = gs.MeasureString(KsWeightTitle, ContentFont);
            gs.DrawString(KsWeightTitle, ContentFont, Brushes.Black, paddingleft + cloum1 + cloum2 + cloum3 * 4 + (cloum3 - KsWeightSizeTitle.Width) / 2, paddingtop + (LineHeight - KsWeightSizeTitle.Height) / 2);
            ...
            foreach (CmcsBuyFuelTransport item in _Transport)
            {
                ...
                if (SupplierName == "合计")
                    ...
                    gs.DrawString(SupplierName, ContentFont, Brushes.Black, paddingleft + (cloum1 - SupplierNameSize.Width) / 2, paddingtop + top + (LineHeight - SupplierNameSize.Height) / 2);
                //煤种
                string FuelKindName = item.FuelKindName;
                ...
                //扣重
                string KsWeight = item.DeductWeight.ToString("F2");
                SizeF KsWeightSize = gs.MeasureString(KsWeight, ContentFont);
                gs.DrawString(KsWeight, ContentFont, Brushes.Black, paddingleft + cloum1 + cloum2 + cloum3 * 4 + (cloum3 - KsWeightSize.Width) / 2, paddingtop + top + (LineHeight - KsWeightSize.Height) / 2);

                //验收量
                string CheckWeight = (item.SuttleWeight - item.DeductWeight).ToString("F2");
