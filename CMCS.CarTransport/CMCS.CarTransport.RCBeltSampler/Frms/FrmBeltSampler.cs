...
using CMCS.CarTransport.RCBeltSampler.Core;
using CMCS.CarTransport.RCBeltSampler.Enums;
using CMCS.CarTransport.RCBeltSampler.Frms.Sys;
...
namespace CMCS.CarTransport.RCBeltSampler.Frms
{
	public partial class FrmBeltSampler : DevComponents.DotNetBar.Metro.MetroForm
	{
		...
		string strWhere = " and BatchType='火车' ";
		...
		/// 采样机编码 默认#1采样机
		/// </summary>
		string[] sampleMachineCodes = new string[] { GlobalVars.MachineCode_InPDCYJ_1 };
		...
		public CmcsCMEquipment CurrentSampleMachine
		{
			...
			set
			{
				...
				if (value != null)
				{
					...
					if (value.EquipmentName.Contains("入厂"))
						strWhere = " and BatchType='火车' ";
					else if (value.EquipmentName.Contains("出厂"))
						strWhere = " and (infactorytype like '%出场' or infactorytype like '%销售%') ";
		...
		public eEquInfGatherType CurrentGatherType
		{
			...
			set
			{
				...
				cmbGatherType.Text = value.ToString();
			}
		}
		...
		private void RefreshEquStatus()
		{
			foreach (UCtrlSignalLight uCtrlSignalLight in flpanEquState.Controls.OfType<UCtrlSignalLight>())
			{
				...
				else if (systemStatus == eEquInfSamplerSystemStatus.正在运行.ToString())
		...
		private void SetGatherType()
		{
			...
			if (Enum.TryParse(CommonDAO.GetInstance().GetCommonAppletConfigString(GlobalVars.MachineCode_OutPDCYJ_1 + "集样方式"), out GatherType))
		...
		{
			List<InfEquInfSampleBarrel> list = BeltSamplerDAO.GetInstance().GetEquInfSampleBarrels(machineCode);
		...
		private void cmbGatherType_SelectedIndexChanged(object sender, EventArgs e)
		{
			eEquInfGatherType gatherType;
			Enum.TryParse(cmbGatherType.Text, out gatherType);
			this.CurrentGatherType = gatherType;
		}
	}
}
