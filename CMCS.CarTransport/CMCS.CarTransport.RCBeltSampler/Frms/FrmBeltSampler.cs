...
namespace CMCS.CarTransport.RCBeltSampler.Frms
{
    public partial class FrmBeltSampler : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        {
            List<View_RCSampling> list = beltSamplerDAO.GetViewRCSampling("where trunc(SamplingDate)=trunc(:SamplingDate) and BatchType='火车' order by  SamplingDate", new { SamplingDate = DateTime.Now.Date });
            superGridControl.PrimaryGrid.DataSource = list;
        }
