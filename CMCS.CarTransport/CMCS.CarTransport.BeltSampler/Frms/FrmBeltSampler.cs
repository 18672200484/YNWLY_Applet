...
namespace CMCS.CarTransport.BeltSampler.Frms
{
    public partial class FrmBeltSampler : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        /// 采样机编码 默认#1采样机
        /// </summary>
        string[] sampleMachineCodes = new string[] { GlobalVars.MachineCode_OutPDCYJ_1 };
        ...
        View_CCSampling currentRCSampling;
        /// <summary>
        /// 当前采样单
        /// </summary>
        public View_CCSampling CurrentRCSampling
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    lblBatch.Text = value.MincoalNo;
                    lblFactarriveDate.Text = value.DraftTime.ToString("yyyy-MM-dd");
                }
                else
                {
                    lblBatch.Text = "####";
        ...
        private void btnSetSampler_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBoxEx.Show("是否设置该记录为当前采样单", "操作提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk) == DialogResult.OK)
                CurrentRCSampling = gridRow.DataItem as View_CCSampling;
        }
        #endregion
