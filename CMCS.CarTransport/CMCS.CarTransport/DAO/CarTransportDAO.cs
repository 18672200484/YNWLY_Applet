...
namespace CMCS.CarTransport.DAO
{
	...
	public class CarTransportDAO
	{
		...
		{
			DateTime dt = SaleFuelTransport.TareTime.Year < 2000 ? SaleFuelTransport.InFactoryTime.AddHours(-commonDAO.GetCommonAppletConfigInt32("汽车分批时间点")) : SaleFuelTransport.TareTime.AddHours(-commonDAO.GetCommonAppletConfigInt32("汽车分批时间点"));
		...
		public CmcsInFactoryBatch GCQCInFactoryBatchBySaleFuelTransport(CmcsSaleFuelTransport buyFuelTransport, CmcsLMYB lmyb)
		{
			...
			{
				// 生成采制化数据记录
				CmcsRCSampling rCSampling = commonDAO.GCSamplingMakeAssay(entity, buyFuelTransport.SampleType, "由汽车煤智能化自动创建", eAssayType.三级编码化验);
				buyFuelTransport.SamplingId = rCSampling.Id;
			}
